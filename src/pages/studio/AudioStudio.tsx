...
export default function AudioStudio() {
  ...
  const [waveformBars, setWaveformBars] = useState<number[]>(Array(50).fill(0.2));
  ...
  useEffect(() => {
    if (!isRecording || isPaused) {
      setWaveformBars(Array(50).fill(0.2));
      return;
    }
  ...
  return (
    <div className="h-screen bg-[#0B0F14] flex flex-col overflow-hidden">
      ...
      <header className="h-14 border-b border-white/10 px-4 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          <h1 className="font-semibold text-white">Audio Studio</h1>
        </div>
        {isRecording && (
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left - Settings */}
        <aside className="w-64 border-r border-white/10 p-4 space-y-6 overflow-y-auto hidden lg:block">
          <div className="space-y-3">
            <h4 className="text-sm font-medium text-white flex items-center gap-2"><Mic className="w-4 h-4" /> Microphone</h4>
            <Button
              variant="ghost"
              onClick={() => setIsMuted(!isMuted)}
              className={cn("w-full h-12 rounded-xl justify-start gap-3", isMuted ? "bg-red-500/20 text-red-400 border border-red-500/30" : "bg-white/5 text-white border border-white/10")}
            >
              {isMuted ? <MicOff className="w-5 h-5" /> : <Mic className="w-5 h-5" />}
              {isMuted ? "Unmute" : "Mute"}
            </Button>
            <div className="space-y-2">
              <div className="flex justify-between text-xs"><span className="text-white/50">Input Level</span><span className="text-white/70">{micLevel}%</span></div>
              <Slider value={[micLevel]} onValueChange={(v) => setMicLevel(v[0])} max={100} step={1} />
            </div>
          </div>

          <div className="space-y-3">
            <h4 className="text-sm font-medium text-white flex items-center gap-2"><Sparkles className="w-4 h-4" /> AI Enhancement</h4>
            <div className="space-y-2">
          ...
          {markers.length > 0 && (
            <div className="space-y-3">
              <h4 className="text-sm font-medium text-white flex items-center gap-2"><Bookmark className="w-4 h-4" /> Markers ({markers.length})</h4>
              <div className="space-y-2">
                {markers.slice(-4).map(m => (
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-6">
          <div className="w-full max-w-2xl p-8 rounded-2xl bg-gradient-to-br from-violet-500/5 via-purple-500/5 to-blue-500/5 border border-white/10">
            <div className="flex items-center justify-center h-32 gap-1 mb-6">
              {waveformBars.map((height, i) => (
                <div
                  key={i}
                  className="flex-1 max-w-1.5 bg-gradient-to-t from-violet-500 to-purple-400 rounded-full transition-all duration-75"
          ...
          <div className="flex items-center gap-4 mt-8">
            {isRecording && (
              <Button variant="outline" size="sm" onClick={() => addMarker("Marker")} className="gap-2 border-white/20 text-white hover:bg-white/10">
              ...
              {!isRecording ? (
                ...
                <>
                  <Button
                    ...
                    className={cn("w-12 h-12 rounded-full", isPaused ? "bg-amber-500/20 text-amber-400" : "bg-white/10 text-white")}
                  >
        ...
        <aside className="w-72 border-l border-white/10 flex flex-col hidden lg:flex">
          <Tabs value={rightTab} onValueChange={setRightTab} className="flex-1 flex flex-col">
            <TabsList className="bg-transparent border-b border-white/10 rounded-none h-12 p-0">
              <TabsTrigger value="scripts" className="flex-1 rounded-none border-b-2 border-transparent data-[state=active]:border-white data-[state=active]:bg-transparent text-white/60 data-[state=active]:text-white">
                <FileText className="w-4 h-4 mr-2" /> Scripts
              </TabsTrigger>
              <TabsTrigger value="transcript" className="flex-1 rounded-none border-b-2 border-transparent data-[state=active]:border-white data-[state=active]:bg-transparent text-white/60 data-[state=active]:text-white">
            ...
            <TabsContent value="scripts" className="flex-1 p-4 m-0">
              <div className="h-full flex flex-col items-center justify-center text-center">
                ...
                <p className="text-white/60 mb-2">Ad Scripts & Host Reads</p>
                <p className="text-xs text-white/40">Add scripts from Templates</p>
              </div>
            </TabsContent>
            <TabsContent value="transcript" className="flex-1 p-4 m-0">
              <div className="h-full flex flex-col items-center justify-center text-center">
                ...
                <p className="text-xs text-white/40">{isRecording ? "Transcribing..." : "Start recording to see live transcript"}</p>
              </div>
            </TabsContent>
          </Tabs>
        </aside>
