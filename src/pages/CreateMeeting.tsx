    ...
    try {
      ...
      if (locationType === 'zoom' && zoomConnected) {
        try {
          ...
          toast({
            ...
            description: "Meeting will be created without Zoom link",
          });
        }
      }
      ...
      if ((createCalendarEvent && calendarConnected) || (locationType === 'meet' && calendarConnected)) {
        try {
          ...
          // If Google Meet was selected, update the location details with the meet link
          if (locationType === 'meet' && calendarData?.htmlLink) {
            toast({
              title: "Google Meet link created",
              description: "Meeting link has been generated and added to calendar",
            });
          }
        } catch (calError) {
          ...
          toast({
            title: "Meeting created, calendar event failed",
            description: "The meeting was created but we couldn't add it to your calendar",
          });
        }
      }

      // Create Microsoft Teams meeting if Teams is selected and Microsoft is connected
      if (locationType === 'teams' && microsoftConnected) {
        try {
          const { data: teamsData, error: teamsError } = await supabase.functions.invoke('microsoft-create-event', {
            body: {
              userId: user?.id,
              meetingTitle: title,
              startTime: startTime.toISOString(),
          ...
          } else if (teamsData?.teamsLink) {
            finalLocationDetails = teamsData.teamsLink;
            
            // Update the meeting record with the Teams link
            await supabase
              .from("meetings")
              .update({ location_details: teamsData.teamsLink })
              .eq("user_id", user?.id)
              .eq("title", title)
